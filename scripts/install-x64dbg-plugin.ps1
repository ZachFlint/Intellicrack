...
$prebuilt64 = "$binDir/intellicrack_bridge_x64.dp64"
$prebuilt32 = "$binDir/intellicrack_bridge_x32.dp32"
$have64 = Test-Path $prebuilt64
$have32 = Test-Path $prebuilt32

if ($have64) { Write-Step 'PLUGIN' "Using prebuilt x64 plugin from $binDir" }
if ($have32) { Write-Step 'PLUGIN' "Using prebuilt x32 plugin from $binDir" }

# Only invoke the CMake toolchain for architectures that lack a committed
# prebuilt binary. A normal install ships both binaries in bin/ and never
# needs a compiler.
$needBuild = (!$have64) -or (!$have32)
$cmakePath = $null
...
if ($needBuild) {
    ...
    if (!$cmakePath) {
        if ($have64 -or $have32) {
            Write-Skip "CMake not found; deploying committed prebuilt binaries only"
        } else {
            Write-Fail "No prebuilt plugin in $binDir and CMake not found. Install Visual Studio or CMake standalone."
            exit 1
        }
    }
}
