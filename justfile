...
radon *FLAGS:
    ...
    @('txt','json','xml') | ForEach-Object { if (!(Test-Path "reports/$_")) { New-Item -ItemType Directory -Path "reports/$_" -Force | Out-Null } }; $tmpFile = [System.IO.Path]::GetTempFileName(); try { pixi run radon cc src/intellicrack -n C -s -a -o SCORE {{ FLAGS }} 2>&1 | Out-File -FilePath $tmpFile -Encoding utf8; pixi run python scripts/process_lint_json.py radon --text $tmpFile } finally { Remove-Item $tmpFile -Force -ErrorAction SilentlyContinue }
...
complexipy *FLAGS:
    ...
    @('txt','json','xml') | ForEach-Object { if (!(Test-Path "reports/$_")) { New-Item -ItemType Directory -Path "reports/$_" -Force | Out-Null } }; $tmpFile = [System.IO.Path]::GetTempFileName(); try { pixi run complexipy {{ FLAGS }} src --failed --color no 2>&1 | Out-File -FilePath $tmpFile -Encoding utf8; pixi run python scripts/process_lint_json.py complexipy --text $tmpFile } finally { Remove-Item $tmpFile -Force -ErrorAction SilentlyContinue }
...
interrogate *FLAGS:
    ...
    @('txt','json','xml') | ForEach-Object { if (!(Test-Path "reports/$_")) { New-Item -ItemType Directory -Path "reports/$_" -Force | Out-Null } }; $tmpFile = [System.IO.Path]::GetTempFileName(); try { pixi run interrogate -vv --fail-under 0 --style google {{ FLAGS }} src/intellicrack 2>&1 | Out-File -FilePath $tmpFile -Encoding utf8; pixi run python scripts/process_lint_json.py interrogate --text $tmpFile } finally { Remove-Item $tmpFile -Force -ErrorAction SilentlyContinue }

# Check for dependency issues with deptry and output sorted findings
deptry *FLAGS:
    @echo "[Deptry] Running..."
    @('txt','json','xml') | ForEach-Object { if (!(Test-Path "reports/$_")) { New-Item -ItemType Directory -Path "reports/$_" -Force | Out-Null } }; $tmpFile = [System.IO.Path]::GetTempFileName(); try { pixi run deptry --no-ansi {{ FLAGS }} . 2>&1 | Out-File -FilePath $tmpFile -Encoding utf8; pixi run python scripts/process_lint_json.py deptry --text $tmpFile } finally { Remove-Item $tmpFile -Force -ErrorAction SilentlyContinue }

# Check for common misspellings with codespell and output sorted findings
codespell *FLAGS:
    @echo "[Codespell] Running..."
    @('txt','json','xml') | ForEach-Object { if (!(Test-Path "reports/$_")) { New-Item -ItemType Directory -Path "reports/$_" -Force | Out-Null } }; $tmpFile = [System.IO.Path]::GetTempFileName(); try { pixi run codespell {{ FLAGS }} src/ tests/ scripts/ docs/ *.md *.toml 2>&1 | Out-File -FilePath $tmpFile -Encoding utf8; pixi run python scripts/process_lint_json.py codespell --text $tmpFile } finally { Remove-Item $tmpFile -Force -ErrorAction SilentlyContinue }
...
taplo *FLAGS:
    @echo "[Taplo] Running..."
    @('txt','json','xml') | ForEach-Object { if (!(Test-Path "reports/$_")) { New-Item -ItemType Directory -Path "reports/$_" -Force | Out-Null } }; $tmpFile = [System.IO.Path]::GetTempFileName(); try { taplo check {{ FLAGS }} 2>&1 | Out-File -FilePath $tmpFile -Encoding utf8; pixi run python scripts/process_lint_json.py taplo --text $tmpFile } finally { Remove-Item $tmpFile -Force -ErrorAction SilentlyContinue }

# Check for mixed line endings and output sorted findings
mixed-line-ending *FLAGS:
    @echo "[Mixed Line Ending] Running..."
    @('txt','json','xml') | ForEach-Object { if (!(Test-Path "reports/$_")) { New-Item -ItemType Directory -Path "reports/$_" -Force | Out-Null } }; $textFiles = @(fd -e py -e md -e txt -e toml -e yaml -e yml -e json -e cfg -e ini -e rst -e sh -e bash -e bat -e cmd -e ps1 --type f --exclude '.pixi' --exclude 'node_modules' --exclude '.git' --exclude 'target' --exclude 'tools' --exclude 'reports' --exclude '*.lock' 2>$null | ForEach-Object { $_.Trim() } | Where-Object { $_ -ne '' }); if ($textFiles.Count -eq 0) { Write-Host "[MIXED-LINE-ENDING] 0 findings (no text files found)"; 'No findings.' | Out-File -FilePath 'reports/txt/mixed-line-ending_findings.txt' -Encoding utf8; @{ tool = 'mixed-line-ending'; generated = (Get-Date).ToString('o'); total_findings = 0; total_files = 0; files = @() } | ConvertTo-Json -Depth 4 | Out-File -FilePath 'reports/json/mixed-line-ending_findings.json' -Encoding utf8; '<?xml version="1.0" encoding="UTF-8"?><LintReport tool="mixed-line-ending"><Summary><TotalFindings>0</TotalFindings><TotalFiles>0</TotalFiles></Summary><Files/></LintReport>' | Out-File -FilePath 'reports/xml/mixed-line-ending_findings.xml' -Encoding utf8; exit 0 }; $tmpFile = [System.IO.Path]::GetTempFileName(); try { $results = @(); foreach ($f in $textFiles) { try { $content = [System.IO.File]::ReadAllBytes($f); $hasCRLF = $false; $hasLF = $false; for ($i = 0; $i -lt $content.Length; $i++) { if ($content[$i] -eq 0x0A) { if ($i -gt 0 -and $content[$i-1] -eq 0x0D) { $hasCRLF = $true } else { $hasLF = $true } } }; if ($hasCRLF -and $hasLF) { $results += "$f`: mixed line endings" } } catch {} }; if ($results.Count -gt 0) { $results | Out-File -FilePath $tmpFile -Encoding utf8 } else { "" | Out-File -FilePath $tmpFile -Encoding utf8 }; pixi run python scripts/process_lint_json.py mixed-line-ending --text $tmpFile } finally { Remove-Item $tmpFile -Force -ErrorAction SilentlyContinue }

# Check for byte-order markers (BOM) in files and output sorted findings
file-encoding *FLAGS:
    @echo "[File Encoding] Running..."
    @('txt','json','xml') | ForEach-Object { if (!(Test-Path "reports/$_")) { New-Item -ItemType Directory -Path "reports/$_" -Force | Out-Null } }; $textFiles = @(fd -e py -e md -e txt -e toml -e yaml -e yml -e json -e cfg -e ini -e rst -e sh -e bash -e bat -e cmd -e ps1 --type f --exclude '.pixi' --exclude 'node_modules' --exclude '.git' --exclude 'target' --exclude 'tools' --exclude 'reports' --exclude '*.lock' 2>$null | ForEach-Object { $_.Trim() } | Where-Object { $_ -ne '' }); if ($textFiles.Count -eq 0) { Write-Host "[FILE-ENCODING] 0 findings (no text files found)"; 'No findings.' | Out-File -FilePath 'reports/txt/file-encoding_findings.txt' -Encoding utf8; @{ tool = 'file-encoding'; generated = (Get-Date).ToString('o'); total_findings = 0; total_files = 0; files = @() } | ConvertTo-Json -Depth 4 | Out-File -FilePath 'reports/json/file-encoding_findings.json' -Encoding utf8; '<?xml version="1.0" encoding="UTF-8"?><LintReport tool="file-encoding"><Summary><TotalFindings>0</TotalFindings><TotalFiles>0</TotalFiles></Summary><Files/></LintReport>' | Out-File -FilePath 'reports/xml/file-encoding_findings.xml' -Encoding utf8; exit 0 }; $tmpFile = [System.IO.Path]::GetTempFileName(); try { $results = @(); foreach ($f in $textFiles) { try { $bytes = [System.IO.File]::ReadAllBytes($f); if ($bytes.Length -ge 3 -and $bytes[0] -eq 0xEF -and $bytes[1] -eq 0xBB -and $bytes[2] -eq 0xBF) { $results += "$f`: Has a byte-order marker (BOM)" } elseif ($bytes.Length -ge 2 -and (($bytes[0] -eq 0xFF -and $bytes[1] -eq 0xFE) -or ($bytes[0] -eq 0xFE -and $bytes[1] -eq 0xFF))) { $results += "$f`: Has a byte-order marker (BOM)" } } catch {} }; if ($results.Count -gt 0) { $results | Out-File -FilePath $tmpFile -Encoding utf8 } else { "" | Out-File -FilePath $tmpFile -Encoding utf8 }; pixi run python scripts/process_lint_json.py file-encoding --text $tmpFile } finally { Remove-Item $tmpFile -Force -ErrorAction SilentlyContinue }
