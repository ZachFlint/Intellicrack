...
vulture *FLAGS:
    ...
    @('txt','json','xml') | ForEach-Object { if (!(Test-Path "reports/$_")) { New-Item -ItemType Directory -Path "reports/$_" -Force | Out-Null } }; $tmpFile = [System.IO.Path]::GetTempFileName(); try { {{ pixi }} vulture {{ FLAGS }} src/ vulture_whitelist.py --min-confidence 60 2>&1 | Out-File -FilePath $tmpFile -Encoding utf8; {{ pixi }} python scripts/process_lint_json.py vulture --text $tmpFile } finally { Remove-Item $tmpFile -Force -ErrorAction SilentlyContinue }
...
dead *FLAGS:
    ...
    @('txt','json','xml') | ForEach-Object { if (!(Test-Path "reports/$_")) { New-Item -ItemType Directory -Path "reports/$_" -Force | Out-Null } }; $tmpFile = [System.IO.Path]::GetTempFileName(); try { {{ pixi }} dead --files 'src/' --symbol-allowlist dead_allowlist.txt {{ FLAGS }} 2>&1 | Out-File -FilePath $tmpFile -Encoding utf8; {{ pixi }} python scripts/process_lint_json.py dead --text $tmpFile } finally { Remove-Item $tmpFile -Force -ErrorAction SilentlyContinue }
...
vermin *FLAGS:
    @echo "[Vermin] Running..."
    @('txt','json','xml') | ForEach-Object { if (!(Test-Path "reports/$_")) { New-Item -ItemType Directory -Path "reports/$_" -Force | Out-Null } }; $tmpFile = [System.IO.Path]::GetTempFileName(); try { {{ pixi }} vermin --no-tips -vvv --target=3.13 --violations {{ FLAGS }} src/ 2>&1 | Out-File -FilePath $tmpFile -Encoding utf8; {{ pixi }} python scripts/process_lint_json.py vermin --text $tmpFile } finally { Remove-Item $tmpFile -Force -ErrorAction SilentlyContinue }
