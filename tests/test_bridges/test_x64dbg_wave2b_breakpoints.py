...
class TestRemoveBreakpointCommandFraming:
    ...
    async def test_bp_remove_issued_with_exact_address(
        ...
        assert ("bp_remove", {"address": _BP_ADDR}) in fake.sent, f"bp_remove must be sent with address={_BP_ADDR!r}; sent={fake.sent!r}"
...
class TestSetWatchpointCommandFraming:
    ...
    async def test_write_watch_type_maps_to_w_access(
        ...
        assert received.get("address") == _WP_ADDR
...
class TestRemoveWatchpoint:
    ...
    async def test_wp_remove_issued_with_watchpoint_address(
        ...
        assert result is True
        assert ("wp_remove", {"address": _WP_ADDR}) in fake.sent, (
            f"wp_remove must be sent with address={_WP_ADDR!r}; got sent={fake.sent!r}"
        )
...
class TestSetBreakpointOnApi:
    ...
    async def test_resolved_path_drives_eval_then_bp_set(
        ...
        assert bp_set_calls[0].get("address") == _API_VA, (
            f"bp_set address must be the VA from eval ({_API_VA!r}), got {bp_set_calls[0].get('address')!r}"
        )
