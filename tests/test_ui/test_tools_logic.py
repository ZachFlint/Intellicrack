...
class TestToolOutputPanelNoDefaultTabs:
    ...
    def test_embedded_tools_dict_starts_empty() -> None:
        ...
        panel = ToolOutputPanel()
        assert len(panel._embedded_tools) == 0
    ...
    def test_tabs_are_closable() -> None:
        ...
        panel = ToolOutputPanel()
        assert panel._tab_widget.tabsClosable() is True
...
class TestLogTabLazyCreation:
    ...
    def test_log_creates_tab_on_first_call() -> None:
        ...
        assert panel._tab_widget.tabText(0) == "Log"
    ...
    def test_log_appends_content() -> None:
        ...
        log_tab = panel._tabs["log"]
        content = log_tab._code_display.toPlainText()
...
class TestTabCloseRequested:
    ...
    def test_close_script_panel_nulls_reference() -> None:
        ...
        tab_index = panel._tab_widget.indexOf(scripts)
        panel._on_tab_close_requested(tab_index)

        assert panel._script_panel is None
        assert "scripts" not in panel._panels
    ...
    def test_close_stack_panel_nulls_reference() -> None:
        ...
        tab_index = panel._tab_widget.indexOf(stack)
        panel._on_tab_close_requested(tab_index)

        assert panel._stack_panel is None
        assert "stack" not in panel._panels
    ...
    def test_close_multiple_tabs_sequentially() -> None:
        ...
        panel._on_tab_close_requested(0)
        assert panel._tab_widget.count() == _EXPECTED_TABS_AFTER_THREE_ADDS - 1

        panel._on_tab_close_requested(0)
        assert panel._tab_widget.count() == _EXPECTED_TABS_AFTER_THREE_ADDS - 2
