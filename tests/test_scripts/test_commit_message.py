...
class TestSubsplitLargeFileDiff:
    ...
    def test_splits_on_hunk_boundaries(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        chunks: list[str] = gcm._subsplit_large_file_diff(diff)
...
class TestSplitDiffOnFileBoundaries:
    ...
    def test_small_diff_single_chunk(self) -> None:
        ...
        chunks: list[str] = gcm._split_diff_on_file_boundaries(diff)
        assert len(chunks) == 1
    ...
    def test_multiple_files_grouped(self) -> None:
        ...
        combined = "".join(diffs)
        chunks: list[str] = gcm._split_diff_on_file_boundaries(combined)
        assert len(chunks) == 1
    ...
    def test_no_chunk_exceeds_target(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        for i, chunk in enumerate(chunks):
            chunk_tokens = gcm._estimate_tokens(chunk)
    ...
    def test_chunks_are_balanced(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        total_est = gcm._estimate_tokens(combined)
        target = total_est // 3
        monkeypatch.setattr(gcm, "CHUNK_TOKEN_TARGET", target)
        ...
        assert len(chunks) >= 2
        sizes = [gcm._estimate_tokens(c) for c in chunks]
...
class TestExtractStatSection:
    ...
    def test_with_markers(self) -> None:
        ...
        inp = "FILES CHANGED:\n file1.py | 5 ++\n\nDIFF:\ndiff --git a/file1.py"
        stat, body = gcm._extract_stat_section(inp)
...
class TestCountTokensFallback:
    ...
    def test_api_error_falls_back_to_estimate(self) -> None:
        ...
        client = MagicMock()
        client.models.count_tokens.side_effect = gcm.ServerError(
            503, {"error": {"message": "unavailable"}}, None
        )
        result = gcm._count_tokens(client, "a" * 300)
        assert result == 100

    def test_client_error_falls_back(self) -> None:
        """Verify that client errors produce an estimate."""
        client = MagicMock()
        client.models.count_tokens.side_effect = gcm.ClientError(
            400, {"error": {"message": "bad request"}}, None
        )
        result = gcm._count_tokens(client, "b" * 600)
        assert result == 200
    ...
    def test_connection_error_falls_back(self) -> None:
        ...
        result = gcm._count_tokens(client, "c" * 900)
        assert result == 300
    ...
    def test_throttle_prevents_rapid_calls(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        gcm._last_count_time[0] = time.monotonic()
        start = time.monotonic()
        gcm._count_tokens(client, "throttle test")
