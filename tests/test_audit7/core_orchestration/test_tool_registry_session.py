...
def test_set_session_none_detaches_all_bridges(tmp_path: Path) -> None:
    """Passing ``None`` to ``set_session`` must detach every bridge.
    ...
    bridge._state.last_error = "post detach"
    bridge._publish_tool_state()

    assert session.tool_states[ToolName.X64DBG].last_error is None
