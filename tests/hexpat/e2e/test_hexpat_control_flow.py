...
class TestNestedControl:
    ...
    def test_conditional_inside_for_loop(self, interp: HexPatInterpreter) -> None:
        ...
        results = interp.execute_bytes(source, data)
        assert len(results) == 6
        one_count = sum(r["name"] == "one" for r in results)
        zero_count = sum(r["name"] == "zero" for r in results)
        assert one_count == 3
        assert zero_count == 3
    ...
    def test_match_inside_while_selects_branch_each_iteration(self, interp: HexPatInterpreter) -> None:
        ...
        results = interp.execute_bytes(source, data)
        type_a = sum(r["name"] == "type_a" for r in results)
        type_b = sum(r["name"] == "type_b" for r in results)
        assert type_a == 2
        assert type_b == 2
