...
class TestDiscoveryCacheTtlExpiry:
    ...
    def test_entry_expires_after_ttl_elapses() -> None:
        ...
        cache.set(provider_ids.OPENAI, [_model(provider_ids.OPENAI, "gpt-x")])
        time.sleep(0.01)
    ...
    def test_fresh_entry_within_ttl_is_returned() -> None:
        ...
        cache.set(provider_ids.ANTHROPIC, models)
        result = cache.get(provider_ids.ANTHROPIC)
        assert result is not None
        assert [m.id for m in result] == ["claude-x"]
        assert cache.is_expired(provider_ids.ANTHROPIC) is False

    @staticmethod
    def test_missing_provider_reports_expired() -> None:
        """A provider that was never cached is reported as expired."""
        cache = DiscoveryCache(ttl_seconds=3600)
        assert cache.is_expired(provider_ids.GOOGLE) is True
    ...
    def test_get_all_cached_excludes_expired_entries() -> None:
        ...
        cache = DiscoveryCache(ttl_seconds=0)
        cache.set(provider_ids.OPENAI, [_model(provider_ids.OPENAI, "a")])
        time.sleep(0.01)
    ...
    def test_invalidate_specific_provider_keeps_others() -> None:
        ...
        cache = DiscoveryCache(ttl_seconds=3600)
        cache.set(provider_ids.OPENAI, [_model(provider_ids.OPENAI, "a")])
        cache.set(provider_ids.GOOGLE, [_model(provider_ids.GOOGLE, "b")])
        cache.invalidate(provider_ids.OPENAI)
        assert cache.get(provider_ids.OPENAI) is None
        google = cache.get(provider_ids.GOOGLE)
...
class TestModelDiscoveryQueryHelpers:
    ...
    def _populate() -> ModelDiscovery:
        ...
        discovery.cache.set(
            provider_ids.OPENAI,
            [
                _model(provider_ids.OPENAI, "gpt-4o", context_window=128000),
                _model(provider_ids.OPENAI, "gpt-4o-mini", context_window=128000),
            ],
        )
        discovery.cache.set(
            provider_ids.ANTHROPIC,
            [_model(provider_ids.ANTHROPIC, "claude-sonnet", context_window=200000)],
        )
        return discovery
    ...
    def test_get_by_id_returns_exact_model(self) -> None:
        ...
        discovery = self._populate()
        model = discovery.get_by_id(provider_ids.ANTHROPIC, "claude-sonnet")
    ...
    def test_get_by_id_unknown_returns_none(self) -> None:
        ...
        discovery = self._populate()
        assert discovery.get_by_id(provider_ids.OPENAI, "no-such-model") is None
    ...
    def test_provider_model_count_reflects_cache(self) -> None:
        ...
        assert counts[provider_ids.ANTHROPIC] == 1
