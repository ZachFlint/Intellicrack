...
def test_validator_validate_method_empty_content_rejected() -> None:
    """Verify validate() rejects empty content instead of faking success.

    An empty string parses cleanly as Python (and JavaScript), so the
    language validators alone would report success when no script is
    present. ``validate`` must guard against this and fail loudly.
    """
    validator = ScriptValidator()
...
def test_validator_validate_method_whitespace_only_rejected() -> None:
    """Verify validate() rejects whitespace-only content for every language."""
    validator = ScriptValidator()
    for language in (ScriptLanguage.PYTHON, ScriptLanguage.JAVASCRIPT, ScriptLanguage.JAVA):
        script = _make_script(language=language, content="   \n\t  \n")
        is_valid, error = validator.validate(script)
        assert is_valid is False, f"whitespace-only {language.value} script must be invalid"
        assert error == "Script is empty"
        assert script.verified is False
