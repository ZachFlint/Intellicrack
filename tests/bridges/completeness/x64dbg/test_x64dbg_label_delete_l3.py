...
"""L3 gate tests for the x64dbg Annotations tab's label Delete control.

``delete_label`` is a new bridge method (registered as
``x64dbg.delete_label``) with a matching "Delete" button next to the
Labels table in ``x64dbg_panel.py``. This module gates the full
click-to-RPC round trip: the button handler must read the selected row's
address, dispatch ``labeldel`` followed by a ``lbl_list`` readback via
the real bridge coroutine, and drop the row once the plugin confirms the
label is gone. It also gates the failure path where the plugin still
reports the label after ``labeldel``, which must surface as a real error
...
_LABEL_ADDR = 0x401000
_LABEL_TEXT = "loop_start"
...
class TestDeleteLabelButtonDrivesDeleteLabelRpc:
    """Clicking Delete must drive ``bridge.delete_label(address)`` and verify absence."""

    @staticmethod
    def test_delete_label_click_issues_labeldel_and_removes_the_row(
        wired_panel: tuple[X64DbgPanel, X64DbgBridge],
        qapp: QApplication,
    ) -> None:
        """Clicking Delete must send ``labeldel 0x401000`` and drop the row once verified gone.

        Falsifiable: if ``_on_delete_label`` read a different table
        column, or ``X64DbgBridge.delete_label`` (``bridges/x64dbg.py``)
        queued a different console command or never read back
        ``lbl_list`` to confirm the label was actually removed, the
        recorded ``exec`` command would not match ``labeldel 0x401000``
        exactly and the row would not disappear from the table.
        ...
        def responder(command: str, params: dict[str, Any] | None) -> dict[str, Any]:
            if command == "exec":
                ...
                assert params.get("command") == f"labeldel {hex(_LABEL_ADDR)}"
                state["deleted"] = True
                return ok("")
            if command == "lbl_list":
                if state["deleted"]:
        ...
        lbl_delete_btn = priv(panel, "_lbl_delete_btn", QPushButton)
        lbl_table = priv(panel, "_lbl_table", QTableWidget)

        try:
            lbl_refresh_btn.click()
            pump_until(qapp, lambda: lbl_table.rowCount() >= 1)
            lbl_table.setCurrentCell(0, 0)

            lbl_delete_btn.click()
            pump_until(qapp, lambda: lbl_table.rowCount() == 0)

            exec_cmds = [p["command"] for c, p in fake.sent if c == "exec" and p]
            assert exec_cmds == [f"labeldel {hex(_LABEL_ADDR)}"]
            assert lbl_table.rowCount() == 0
            assert lbl_delete_btn.isEnabled()
        finally:
            panel.deleteLater()

    @staticmethod
    def test_delete_label_that_did_not_take_reports_failure_not_fabricated_success(
        wired_panel: tuple[X64DbgPanel, X64DbgBridge],
        qapp: QApplication,
    ) -> None:
        """A ``lbl_list`` that still reports the label after ``labeldel`` must surface as an error.

        Falsifiable: a naive implementation that returns ``{"success":
        True, ..., "verified": False}`` without checking ``lbl_list``'s
        readback would report success here even though the plugin's own
        label listing proves the delete never took effect; this test
        only passes when ``delete_label`` actually raises ``ToolError``
        ...
        try:
            ...
            lbl_delete_btn.click()
            pump_until(qapp, lambda: "Delete Label failed" in console_output.toPlainText())

            assert "Delete Label failed" in console_output.toPlainText()
