...
"""L3 gate tests for the x64dbg Advanced tab's breakpoint log-condition control.

``set_breakpoint_log_condition`` is a fully implemented and registered bridge
method (``x64dbg.set_breakpoint_log_condition``) with a matching "Set Log
Condition" control on the BP Config sub-tab in ``x64dbg_advanced_tab.py``.
This module gates the full click-to-RPC round trip for both the default
software-breakpoint dispatch and the type-branched dispatch for a breakpoint
...
class TestSetBreakpointLogConditionButtonDrivesTypeBranchedRpc:
    """Clicking Set Log Condition must branch its command family by breakpoint type."""

    @staticmethod
    def test_default_software_breakpoint_sends_software_log_condition_command(
        wired_tab: tuple[X64DbgAdvancedTab, X64DbgBridge],
        qapp: QApplication,
    ) -> None:
        """An address this bridge never recorded must default to the software command.

        Falsifiable: if ``X64DbgBridge.set_breakpoint_log_condition``
        (``bridges/x64dbg.py``) did not resolve the breakpoint type via
        ``_resolve_breakpoint_type``, or the Advanced tab's
        ``_on_set_breakpoint_log_condition`` read a different
        address/condition widget, the recorded ``exec`` command would
        not match ``SetBreakpointLogCondition 0x401000, "eax==1"``
        exactly.
        ...
        def responder(command: str, params: dict[str, Any] | None) -> dict[str, Any]:
            if command == "exec":
                ...
                assert params.get("command") == 'SetBreakpointLogCondition 0x401000, "eax==1"'
                return ok("")
        ...
        logcond_input = priv(tab, "_bpcfg_logcond_input", QLineEdit)
        logcond_btn = priv(tab, "_bpcfg_logcond_btn", QPushButton)
        status_label = priv(tab, "_bpcfg_status_label", QLabel)

        addr_input.setText("0x401000")
        logcond_input.setText("eax==1")
        logcond_btn.click()
        pump_until(qapp, lambda: "Log condition set" in status_label.text())

        exec_cmds = [p["command"] for _, p in fake.sent if p and "command" in p]
        assert 'SetBreakpointLogCondition 0x401000, "eax==1"' in exec_cmds
        assert "Log condition set at 0x401000" in status_label.text()
        assert logcond_btn.isEnabled()

    @staticmethod
    def test_hardware_breakpoint_sends_hardware_log_condition_command(
        wired_tab: tuple[X64DbgAdvancedTab, X64DbgBridge],
        qapp: QApplication,
    ) -> None:
        """An address this bridge recorded as hardware must branch to the hardware command.

        Falsifiable: this is the case a hardcoded software-only command
        cannot pass. If ``set_breakpoint_log_condition`` sent
        ``SetBreakpointLogCondition`` regardless of ``bp_type``, the
        recorded command would not match
        ``SetHardwareBreakpointLogCondition 0x401000, "eax==1"``.
        ...
        def responder(command: str, params: dict[str, Any] | None) -> dict[str, Any]:
            if command == "exec":
                assert params is not None
                assert params.get("command") == 'SetHardwareBreakpointLogCondition 0x401000, "eax==1"'
                return ok("")
            msg = f"unexpected command: {command}"
            raise AssertionError(msg)
        ...
        exec_cmds = [p["command"] for _, p in fake.sent if p and "command" in p]
        assert 'SetHardwareBreakpointLogCondition 0x401000, "eax==1"' in exec_cmds
        assert "Log condition set at 0x401000" in status_label.text()
        assert logcond_btn.isEnabled()
