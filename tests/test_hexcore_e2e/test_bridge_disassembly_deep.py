...
class TestDisassemblePeTextSection:
    ...
    def test_disassemble_pe_text_int3_mnemonic(
        ...
        results: list[dict[str, Any]] = _run(
            loaded_bridge.disassemble(_PE_TEXT_OFFSET, count=4, arch="x86", mode="64")
        )
        assert len(results) > 0
        assert results[0]["mnemonic"] == "int3"

    def test_disassemble_pe_text_count_1_returns_exactly_one(
        self, loaded_bridge: Any
    ) -> None:
        ...
        """
        results: list[dict[str, Any]] = _run(
            loaded_bridge.disassemble(_PE_TEXT_OFFSET, count=1, arch="x86", mode="64")
        )
        assert len(results) == 1

    def test_disassemble_pe_text_count_4_returns_up_to_4(
        self, loaded_bridge: Any
    ) -> None:
    ...
    def test_disassemble_pe_text_address_equals_offset(
        self, loaded_bridge: Any
    ) -> None:
    ...
    def test_disassemble_pe_text_all_required_keys_present(
        self, loaded_bridge: Any
    ) -> None:
    ...
    def test_disassemble_pe_text_with_auto_arch(
        self, loaded_bridge: Any
    ) -> None:
        ...
        """
        results: list[dict[str, Any]] = _run(
            loaded_bridge.disassemble(_PE_TEXT_OFFSET, count=4, arch="auto")
        )
        assert isinstance(results, list)

    def test_disassemble_pe_text_explicit_x86_64_matches_auto(
        self, loaded_bridge: Any
    ) -> None:
        ...
        Args:
            loaded_bridge: Bridge with a PE file already loaded.
        """
        auto_results: list[dict[str, Any]] = _run(
            loaded_bridge.disassemble(_PE_TEXT_OFFSET, count=1, arch="auto")
        )
        explicit_results: list[dict[str, Any]] = _run(
            loaded_bridge.disassemble(_PE_TEXT_OFFSET, count=1, arch="x86", mode="64")
        )
...
class TestDisassembleMzHeader:
    ...
    def test_disassemble_at_mz_header_does_not_crash(
        self, loaded_bridge: Any
    ) -> None:
        ...
        """
        results: list[dict[str, Any]] = _run(
            loaded_bridge.disassemble(0, count=4, arch="x86", mode="64")
        )
        assert isinstance(results, list)

    def test_disassemble_mz_header_address_starts_at_zero(
        self, loaded_bridge: Any
    ) -> None:
...
class TestDisassembleX86Mode32:
    ...
    def test_disassemble_with_mode_32_returns_instructions(
        ...
        results: list[dict[str, Any]] = _run(
            bridge.disassemble(0, count=4, arch="x86", mode="32")
        )
        assert isinstance(results, list)
        assert len(results) > 0
...
class TestDisassembleKnownX86Sequence:
    ...
    def test_nop_nop_nop_int3_sequence(
        self, bridge: Any, tmp_path: Path
    ) -> None:
        ...
        _run(bridge.open_file(str(f)))
        results: list[dict[str, Any]] = _run(
            bridge.disassemble(0, count=4, arch="x86", mode="64")
        )
    ...
    def test_nop_bytes_field_is_valid_hex_string(
        self, bridge: Any, tmp_path: Path
    ) -> None:
        ...
        _run(bridge.open_file(str(f)))
        results: list[dict[str, Any]] = _run(
            bridge.disassemble(0, count=2, arch="x86", mode="64")
        )
        assert len(results) > 0
    ...
    def test_nop_size_field_is_positive_int(
        self, bridge: Any, tmp_path: Path
    ) -> None:
    ...
    def test_int3_bytes_field_equals_cc_hex(
        self, bridge: Any, tmp_path: Path
    ) -> None:
        ...
        _run(bridge.open_file(str(f)))
        results: list[dict[str, Any]] = _run(
            bridge.disassemble(0, count=1, arch="x86", mode="64")
        )
        assert len(results) == 1
...
class TestDisassembleEdgeCases:
    ...
    def test_disassemble_at_end_of_file_returns_empty_or_partial(
        self, loaded_bridge: Any, pe_bytes: bytes
    ) -> None:
        ...
        results: list[dict[str, Any]] = _run(
            loaded_bridge.disassemble(near_end, count=10, arch="x86", mode="64")
        )
        assert isinstance(results, list)
        assert len(results) <= 1
