...
class TestVNCWidget:
    ...
    def test_connect_to_unreachable_emits_false(qapp: QApplication) -> None:
        ...
        widget.connect_to_server("127.0.0.1", VNC_PORT_UNREACHABLE, timeout=CONNECT_TIMEOUT_SEC)

        deadline = time.monotonic() + CONNECT_TIMEOUT_SEC + CONNECT_WAIT_SEC
    ...
    @staticmethod
    def test_dispatched_worker_not_parented_to_widget(qapp: QApplication) -> None:
        """A dispatched bridge worker must not be a Qt child of the widget.

        ``disconnect_from_server`` starts a background ``BridgeCallWorker``
        ``QThread``. If that worker were parented to the widget, destroying the
        widget while the worker's OS thread is still running would trigger Qt's
        ``QThread: Destroyed while thread is still running`` process abort - the
        exit-255 crash observed when the full suite is run in a single process
        under coverage. Leaving the worker unparented hands its lifetime to the
        retention registry so the widget can be torn down mid-flight safely.

        Args:
            qapp: Qt application fixture used to pump the deferred-delete queue.
        """
        widget = VNCWidget()
        widget.disconnect_from_server()
