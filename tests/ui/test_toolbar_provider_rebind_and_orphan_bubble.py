...
"""Regression gates for S16-D01 (toolbar rebind) and S16-D05 (orphan bubble).
...
Both tests drive the real :class:`MainWindow` over a real
:class:`Orchestrator` and real, self-contained :class:`LLMProviderBase`
subclasses -- no mocked provider calls or asserted-on mocks. Streaming is
disabled on the orchestrator config so both tests exercise the non-streaming
completion path described by the defects.
"""
