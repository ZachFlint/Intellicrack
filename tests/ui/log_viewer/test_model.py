...
def test_column_data_for_display_role() -> None:
    ...
    The structlog internal keys (timestamp, level, logger, event, module,
    function, line_number) must not appear in the extras mapping.
    """
    ...
    assert isinstance(extras_text, str), "extras column must return a str"
    extras_parsed: object = json.loads(extras_text)
    assert isinstance(extras_parsed, dict), "extras column must be valid JSON object"
    assert extras_parsed == {"widget": "x", "count": 3, "active": True}, (
        f"extras dict must contain exactly the user-supplied extras; got {extras_parsed!r}"
    )
    assert isinstance(extras_parsed["count"], int), "int extras must round-trip as int"
    assert extras_parsed["active"] is True, "bool extras must round-trip as bool"
    assert "\n" not in extras_text, "extras column must not contain newlines"
    assert "\t" not in extras_text, "extras column must not contain tabs"
