...
from intellicrack.core.types import ToolState
...
TOOL_CAPABILITY_MAP: dict[str, str] = {
    ...
    "get_registers": "debugging",
    "set_register": "debugging",
    "get_threads": "debugging",
    "get_modules": "debugging",
...
``BridgeCapabilities`` as ``supports_<value>``. ``ToolRegistry`` consults this mapping in ``execute_tool_call`` and prefers a fully-
qualified match over the short-name match, so a bridge whose method name collides with an unrelated bridge's (for example the sandbox's
``stop`` versus a debugger's ``stop``) can require the capability that actually fits its own operation instead of inheriting the short
name's capability. It raises ``ToolError`` when a bridge is asked to execute a tool whose required capability it does not advertise. The
...
class ToolBridgeBase(ABC):
    ...
    def set_session(self, session: Session | None) -> None:
        ...
        When a session is attached, the bridge immediately publishes its
        current ``BridgeState`` as a ``ToolState`` entry on the session so
        the persisted view reflects any state changes that happened before
        the session was wired in. Passing ``None`` detaches the bridge
        without modifying the previously held session.

        Args:
            session: The active ``Session`` whose ``tool_states`` registry
    ...
    def _publish_tool_state(self) -> None:
        ...
        state = ToolState(
            ...
            process_attached=self._state.process_attached,
    ...
    def _clear_tool_state_in_session(self) -> None:
        ...
        session.clear_tool_state(self.name)
    ...
    async def _finalize_shutdown(self) -> None:
        ...
        self._clear_tool_state_in_session()
